class Declaration < PaymentRecord
    validates :ticket_number, uniqueness: true

    def notification_data
        {
            #msisdn: self.msisdn,
            codeDeclarant: self.declarant_code,
            reference_id: self.payment_trnx_ref,
            montant: self.amount.to_s,
            codeNif: self.nif_code,
            reference: self.liquidation_number,
            moyen_paiement: humanize_wallet,
        }
    end

    def humanize_wallet
        return wallet = self.wallet == 'mtn_guinee' ? 'mtn' : 'orange'
    end
    ...
    def self.filter_columns
      ...
      def self.export_attributes
        ['ID', 'TICKET', 'CODE DECLARANT', 'CODE NIF', 'NUMERO LIQUIDATION', 'NUMERO QUITTANCE', 'MONTANT', 'FRAIS', 'TELEPHONE', 'MOYEN DE PAIEMENT', 'STATUT', 'REFERENCE DE PAIEMENT', 'DATE']
      end
      ...
      def sms_content
        if self.status == "success" 
            %(Le paiement #{self.ticket_number} de la declaration #{self.liquidation_number}, mumero de quittance #{self.quittance_number} a été effectué avec succès!)
        else
            %(Le paiement de la declaration #{self.liquidation_number} a échoué! Veuillez réessayer plus tard !")
        end
      end
